...
(function () {
    ...
    var adFilter = function (AD_OBJECT) {
        ...
        var sortedArray = AD_OBJECT.filter(checkTypeField).filter(checkPriceField).filter(checkRoomsField).filter(checkGuestsField).filter(checkFeaturesField);
        return sortedArray;
    };
    window.filter = {
        adFilter: adFilter
